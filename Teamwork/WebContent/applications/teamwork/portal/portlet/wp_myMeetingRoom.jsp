...
com.twproject.task.MeetingRoom,
                 ...
                 net.sf.json.JSONArray, org.hibernate.Query, org.jblooming.agenda.CompanyCalendar, org.jblooming.oql.OqlQuery,
...
<div class="portletBox myIssue"><%
  ...
  ps.addClientEntry(Fields.FLD_FILTER_NAME, "PF_MY_OPEN_MEETINGROOM");
  ...
  bs.onClickScript = "$('#myMeetingRoom').toggle()";
  bs.iconChar="g";
  bs.label="";
  ...
  <%
    sub.label=I18n.get("MYMEETINGROOM");
    sub.iconChar="";
  %>
  ...
  <div id="myMeetingRoom" class="portletParams" style="display:none"><%

  int maxIssuesToShow = pageState.getEntryOrDefault("MAX_MY_MEETINGROOM_DISPLAY", "8").intValue();

  TextField tf = new TextField("MAX_MY_MEETINGROOM_DISPLAY","&nbsp;");
  tf.label=I18n.get("MAX_MEETINGROOM_DISPLAY");
  tf.fieldSize=2;
  tf.script=" onBlur=\"refreshPortlet($(this),{'MAX_MY_MEETINGROOM_DISPLAY':$(this).val()});\"";
...
%>
  ...
  <table class="table dataTable" >
    <thead>
    <tr>
      <th class="tableHead" width="1%"></th>
      <th class="tableHead">会议室编号</th>
      <th class="tableHead">会议室名称</th>
      <th class="tableHead" colspan="2"><%=I18n.get("ISSUE_TASK")%></th>
      <th class="tableHead" nowrap>会议室状态</th>
      <th class="tableHead" nowrap></th>
      <th class="tableHead" nowrap></th>
    </tr>
    </thead>
    <tbody id="wp_myMeetingRoomExpired" style="display:none;"></tbody>
    <tbody id="wp_myMeetingRoom"></tbody>
  </table>

  <%
    if (issues.size()==0 && visitedIds.size() ==0) {
  %><div class="hint"><%=I18n.get("NO_MEETINGROOM")%></div><%
    }
  %>
...
<script type="text/javascript">
  ...
  $.when.apply(null, defs) // per chiamare la when con un array [a,b,c,d] invece che parametri sep.  (a,b,c,d)
    .done(function () {
      //console.debug("wp_myMeetingRoom : all loaded")
      $("#wp_myIssueTemplates").loadTemplates().remove();

      var expired =<%=expiredIssues%>;
      var issues =<%=jsIssues%>;

      var ndo = $("#wp_myMeetingRoomExpired");
      ...
      //bind  issueEvent
      registerEvent("issueEvent.wpMyMeetingRoom", function (e, data) {
        //console.debug("wp_myMeetingRoom",data);
        refreshPortlet($("#myMeetingRoom"));
      });
