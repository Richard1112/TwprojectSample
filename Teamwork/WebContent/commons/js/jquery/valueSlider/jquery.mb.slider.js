	...
	$.mbSlider = {
		...
		buildSlider: function (options) {
			return this.each(function () {
				...
				var sliderHandlerPos = startPos - (startPos > slider.sliderHandler.outerWidth() ? slider.sliderHandler.outerWidth() : 0);

				slider.sliderHandler.css({left: sliderHandlerPos });
				slider.sliderValueLabel.css({left: sliderHandlerPos - (slider.sliderValueLabel.width()/2) });

				if (this.options.startAt > 0) {

					slider.sliderRange.css({left: 0, width: sliderHandlerPos });
				...
				if(sliderHandlerPos >= slider.sliderRange.width()) {

					slider.sliderValueLabel.css({left: sliderHandlerPos - (slider.sliderValueLabel.outerWidth()) + (slider.sliderHandler.outerWidth()) });
				...
				slider.sliderBar.on("mousedown.mb_slider", function (e) {
					...
					var sliderHandlerPos = posInGrid > slider.sliderHandler.outerWidth() ? posInGrid - (slider.sliderHandler.outerWidth()/2) : 0;
					...
					$(document).on("mousemove.mb_slider", function (e) {
						...
						if (slider.evalPosGrid >= slider.options.maxVal) {
						...
						} else if (slider.evalPosGrid <= slider.options.minVal || mousePos <= slider.sliderHandler.outerWidth()) {
							...
							var sliderHandlerPos = posInGrid - (slider.sliderHandler.outerWidth());

							slider.sliderHandler.css({left: sliderHandlerPos});
							slider.sliderValueLabel.css({left: sliderHandlerPos - (slider.sliderValueLabel.outerWidth()/2) + (slider.sliderHandler.outerWidth()/2) });
						...
						if(!sliderHandlerPos && slider.sliderRange.width() > 0) {
							slider.sliderValueLabel.css({left: slider.sliderRange.width() - (slider.sliderValueLabel.outerWidth()) + (slider.sliderHandler.outerWidth()) });
						}
